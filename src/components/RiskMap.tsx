    ...
    <div className='space-y-8'>
      <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-4'>
        {risks.map((risk) => (
          ...
          >
            <div
              className={`text-[10px] font-mono px-2 py-0.5 rounded-full border w-fit mb-3 ${getPriorityColor(risk.sev)}`}
            >
              {risk.sev} | {risk.cat}
            </div>
            <h4 className='font-bold leading-tight'>{risk.title}</h4>
            <div
              className={`absolute bottom-2 right-4 text-4xl font-mono opacity-10 ${selected.includes(risk.id) ? 'opacity-20' : ''}`}
            >
              {risk.id}
            </div>
          </button>
      ...
      {selected.length > 0 && (
        <motion.div
          ...
          className='p-8 bg-black border border-zinc-800 rounded-3xl'
        >
          <div className='flex flex-col md:flex-row gap-8 items-center'>
            <div className='flex-1'>
              <h3 className='text-xl font-bold mb-2'>Анализ выбранного профиля рисков</h3>
              <p className='text-zinc-500 text-sm'>
                Мы сформировали базовый план компенсационных мероприятий для ваших P1-рисков.
              </p>
            </div>
            <div className='flex gap-4'>
              <div className='text-center'>
                <div className='text-3xl font-bold text-rose-500'>
                  {selected.filter((id) => risks.find((r) => r.id === id)?.sev === 'P1').length}
                </div>
