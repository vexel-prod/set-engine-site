...
const Calculator: React.FC = () => {
  ...
  const estimatedMin = (formData.area * baseRate * regionMultiplier * urgencyMultiplier * 0.9) / 1000000;
  const estimatedMax = (formData.area * baseRate * regionMultiplier * urgencyMultiplier * 1.1) / 1000000;

  const estimatedMonths = Math.ceil(formData.area / 1000) * (formData.urgency === 'urgent' ? 0.7 : 1);
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-12 items-start bg-zinc-900 border border-zinc-800 p-8 rounded-3xl">
      <div className="space-y-6">
        ...
        <div>
          <label className="block text-sm text-zinc-500 mb-2 font-mono uppercase">Тип объекта</label>
          <select 
            value={formData.type}
        ...
        <div>
          <label className="block text-sm text-zinc-500 mb-2 font-mono uppercase">Площадь (м²): {formData.area}</label>
          <input 
      ...
      <div className="bg-black/50 p-8 rounded-2xl border border-brand-500/20 flex flex-col justify-center min-h-[400px]">
        <div className="text-center space-y-8">
          <div>
            <span className="text-sm font-mono text-zinc-500 uppercase tracking-widest block mb-4">Оценочный бюджет</span>
            <div className="text-4xl md:text-5xl font-bold font-display text-white mb-2">
              {estimatedMin.toFixed(1)} – {estimatedMax.toFixed(1)} <span className="text-2xl text-zinc-500 font-normal">млн ₽</span>
            </div>
            <p className="text-xs text-zinc-500 mt-2">* Расчет носит предварительный характер и не является офертой</p>
          </div>

          <div className="h-px bg-zinc-800 w-full mx-auto" />

          <div>
            <span className="text-sm font-mono text-zinc-500 uppercase tracking-widest block mb-4">Срок реализации</span>
            <div className="text-4xl font-bold font-display text-brand-500">
              {estimatedMonths} – {Math.ceil(estimatedMonths * 1.4)} <span className="text-xl font-normal text-zinc-500 uppercase tracking-tighter">Мес.</span>
            </div>
          </div>
